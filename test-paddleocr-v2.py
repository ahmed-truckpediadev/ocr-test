"""PaddleOCR proper test - using correct dict-like API"""
import os
import sys
import time
import re
...
PDF_GROUPS = {
    ...
    "Truck Center (Scanned, 2pg)": ["invoice2-1.png", "invoice2-2.png"],
...
for pdf_name, pages in PDF_GROUPS.items():
    ...
    for page in pages:
        ...
        for res_item in result:
            # OCRResult is dict-like - access via keys
            ...
            for i, text in enumerate(rec_texts):
                page_text += text + "\n"
                if i < len(rec_scores):
                    page_scores.append(float(rec_scores[i]))

        avg_score = sum(page_scores) / len(page_scores) if page_scores else 0
        print(f"  Page {page}: {len(page_scores)} boxes, avg: {avg_score:.1%}, time: {elapsed:.1f}s")
        sys.stdout.flush()
        full_text += page_text

    print(f"\nTotal time: {total_time:.1f}s | Text: {len(full_text)} chars")
    ...
    print(f"\n--- QUALITY ---")
    print(f"VIN: {'✅' if has_vin else '❌'} | $: {'✅' if has_dollar else '❌'} | Dates: {'✅' if has_date else '❌'}")
    print("")
    sys.stdout.flush()
